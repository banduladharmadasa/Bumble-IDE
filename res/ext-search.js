...
hyperace.hypersearch.prototype = {
    ...
    _search: function (session, s) {
		...
		//we don't call below method as it raise a security warning
		//window.external.OnFindResultReady(this.target.innerHTML.toString());
		//here we inform c++ that the data is available without 
...
function loaded() {
    ...
    var html = "<div id='appendopt'> <div id='hyperopt'> <header>Hypersearch options [tab] to activate</header> <label for='hyperall'>Search All</label> <input type='radio' id='hyperall' name='hyperwho' checked='checked'/> &nbsp;&nbsp;&nbsp; <label for='hypercur'>Search Current</label> <input type='radio' id='hypercur' name='hyperwho' /> </div>  </div>"
    var nodeList = document.querySelectorAll('.ace_search');
