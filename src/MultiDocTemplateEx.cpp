...
CDocTemplate::Confidence CMultiDocTemplateEx::MatchDocType(const WCHAR *pszPathName, CDocument *&rpDocMatch)
{
		...
		!strFilterExt.IsEmpty())
	{
		...
		if (-1 != nSemi)
		{
			...
			if (nDot >= 0 && (lstrcmpi(pszPathName + nDot, ext1) == 0 || lstrcmpi(pszPathName + nDot, ext2) == 0))
